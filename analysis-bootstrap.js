(function(){
  ...
  async function boot(){
    ...
    window.LIKEWHAT_PATTERNS=catalog.records;
    window.LIKEWHAT_ANALYSIS_DATA={catalog,page};

    await script('design-space.js');
    await script('vocabulary.js');

    if(page==='map'){
      await loadPreviewStack();
      await script('map.js');
    }else if(page==='vocabulary'){
      await loadPreviewStack();
      await script('vocabulary-page.js');
    }else if(page==='compare'){
      const cache=new Map();
      window.LikeWhatDetailStore={
        async get(id){
          if(cache.has(id))return cache.get(id);
          const record=catalog.records.find(p=>p.id===id);
          if(!record)throw new Error(`Unknown pattern ${id}`);
          const promise=json(`generated/patterns/${encodeURIComponent(record.detailFile)}`).then(detail=>{
            const index=window.LIKEWHAT_PATTERNS.findIndex(p=>p.id===id);
            if(index>=0)window.LIKEWHAT_PATTERNS[index]=detail;
            return detail;
          });
    ...
    window.LikeWhatAnalysisLoadMetrics={
      ...
      fullDetailRecords:page==='compare'?(window.LikeWhatDetailStore?.size?.()||0):0,
  ...
  boot().catch(error=>{
    console.error('[Like What?] analysis bootstrap failed',error);
    document.querySelector('main')?.insertAdjacentHTML('afterbegin','<section class="not-found"><h1>Analysis could not load.</h1><p>分析用カタログの読み込みに失敗しました。</p><a href="./">TOPへ戻る</a></section>');
  });
})();
