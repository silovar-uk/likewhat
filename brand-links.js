(function(){
  ...
  function scoreSource(url){
    try{
      ...
      if(!/^https?:$/.test(u.protocol))return Number.POSITIVE_INFINITY;
      const path=u.pathname.replace(/\/+$/,'');
      const segments=path.split('/').filter(Boolean).length;
      const text=`${u.hostname}${path}`.toLowerCase();
      let score=segments*20+path.length;
      if(/\b(help|support|docs?|documentation|guidelines?|manual)\b/.test(text))score+=24;
      if(/developer\./.test(u.hostname))score+=10;
      return score;
  ...
  [...new Set(patterns.map(p=>p.brand))].forEach(brand=>{
    const candidates=patterns
      .filter(p=>p.brand===brand&&p.sourceUrl)
      .map(p=>p.sourceUrl)
  ...
  function apply(root=document){
    root.querySelectorAll?.('.pattern-card[data-brand]').forEach(card=>{
      wire(card.querySelector('.card-meta span:first-child'),card.dataset.brand);
    });
  ...
  document.addEventListener('click',event=>{
    const target=event.target.closest?.('[data-brand-external]');
    if(target)openBrand(target,event);
  });
  document.addEventListener('keydown',event=>{
    if(event.key!=='Enter'&&event.key!==' ')return;
    const target=event.target.closest?.('[data-brand-external][role="link"]');
    if(target)openBrand(target,event);
  });
