(function(){
  ...
  function notFound(){
    document.title='Pattern not found — Like What?';
    root.innerHTML='<section class="not-found"><h1>Pattern not found.</h1><p>指定された参照は現在のカタログにありません。</p><a href="./#patterns">一覧へ戻る</a></section>';
  }
  ...
  async function boot(){
    ...
    if(!record){notFound();return;}
    const detail=await json(`generated/patterns/${encodeURIComponent(record.detailFile)}`);
    window.LIKEWHAT_PATTERNS=catalog.records.map(item=>item.id===id?detail:{...item});
    window.LIKEWHAT_PATTERN_DATA={catalog,selected:detail};
    ...
    await script('pattern.js');
    await script('cluster-detail.js');
    await script('pattern-idol.js');
    await script('brand-links.js');
    await script('discovery-v2.js');

    window.LikeWhatPatternLoadMetrics={
      selectedId:id,
      referenceCount:catalog.referenceCount,
      fullDetailRecords:1,
      compactRecords:catalog.records.length-1,
  ...
  boot().catch(error=>{
    console.error('[Like What?] pattern bootstrap failed',error);
    root.innerHTML='<section class="not-found"><h1>Pattern could not load.</h1><p>詳細データの読み込みに失敗しました。</p><a href="./#patterns">一覧へ戻る</a></section>';
  });
})();
