(function(){
  ...
  function notFound(){
    document.title='Brand not found — Like What?';
    root.innerHTML='<section class="brand-not-found"><h1>Brand not found.</h1><p>指定されたブランド／アーティストは現在のカタログにありません。</p><a href="./#patterns">一覧へ戻る</a></section>';
  }

  async function boot(){
    if(!brand){notFound();return;}
    root.innerHTML='<section class="brand-not-found"><p class="eyebrow">BRAND / ARTIST VIEW</p><h1>Loading this collection…</h1><p>このブランドに属するPatternだけを取得しています。</p></section>';

    const [catalog,index]=await Promise.all([
      json('generated/catalog.json'),
      json('generated/brands/index.json')
    ]);
    const entry=index.brands.find(item=>item.brand===brand);
    ...
    window.LIKEWHAT_PATTERNS=details;
    window.LIKEWHAT_BRAND_DATA={catalog,manifest,details};
    ...
    await script('brand.js');
    await script('brand-idol.js');

    window.LikeWhatBrandLoadMetrics={
      brand,
      type:manifest.type,
      referenceCount:catalog.referenceCount,
      fullDetailRecords:details.length,
      skippedFullDetailRecords:Math.max(0,catalog.referenceCount-details.length),
  ...
  boot().catch(error=>{
    console.error('[Like What?] brand bootstrap failed',error);
    root.innerHTML='<section class="brand-not-found"><h1>Brand could not load.</h1><p>Brand / Artistデータの読み込みに失敗しました。</p><a href="./#patterns">一覧へ戻る</a></section>';
  });
})();
